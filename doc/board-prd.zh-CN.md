...
- Owner 可以编辑项目、配置状态、管理标签和成员、查看 Project Activity、转移所有权、归档项目及删除项目。
- 项目列表分页加载，仅加载当前页项目的子记录。

### 项目归档

- Owner 可从项目选项菜单归档活动项目。项目会立即离开分页的活动 Board，同时保留项目偏好和全部数据。
- Board 顶部可打开参考 Docs 归档设计的分页弹窗。弹窗列出当前用户可见的归档项目，但只有项目 Owner 能看到恢复和永久删除操作。
- 恢复后项目依据保留的排序偏好重新进入活动 Board；永久删除沿用现有的 Owner-only 项目删除行为。
- 归档项目仍可通过明确包含归档内容的查询读取，但共享命令层会拒绝项目、流程、成员和任务写入，直至 Owner 恢复项目。
- 归档与恢复通过需要认证且仅限 Owner 的接口执行，并在 Project Activity 中以 `update_project` 的 `archived` 变化记录。
...
- 只有 Owner 可以归档、恢复或永久删除项目；归档项目不出现在活动 Board 中，并保留排序偏好供恢复使用。
