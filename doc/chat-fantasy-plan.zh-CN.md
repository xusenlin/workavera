...
- Provider 完全不上报输入用量时，上下文快照退回为按发送历史与本轮产出的字符估算，并打上 estimated 标记，UI 以 `~` 前缀展示。
...
- Reading：搜索、读取、写入和总结。
- Docs：搜索、读取、带乐观并发控制的写入、精确文本替换和长内容分块写入（Markdown 或自包含 HTML 文档）。

Board 任务搜索可在不提供项目 ID 时按标题或描述关键词查询，范围为当前用户可见的活动项目，并在每条命中中直接返回任务 ID、所属项目和完整状态；调用方仍可按项目、状态或负责人限定，也可显式包含归档项目。

工具描述要求具备明确写入意图、使用前置读取获得的真实 ID、在适用场景提交最新 revision，并对同一资源串行写入。支持批量的写入工具通过必填 `items` 数组接收 1 至 50 条记录，并按顺序返回逐条结果；单条失败不会撤销同批次中已成功的记录。该数组契约适用于 Board 状态、标签、成员、任务创建和任务更新，Calendar 事件创建/更新、Reading upsert 与 Docs 移动。新的执行不接受旧的顶层单记录输入。

工具结果使用模块专属 UI 卡片渲染，并可通过统一工作区深链接打开记录。批量结果统一展示总数、成功数、失败数和有序的逐条详情。历史中已持久化的旧单条结果仍可查看，但不能作为新执行的输入格式。Mock 工具不会进入生产注册表。
...
- 历史消息行与状态稳定的工具卡使用 memo；流式期间只有当前 Assistant 回答重新渲染，除非关联运行或审批状态发生变化。
- 运行中、成功、失败和审批工具卡默认折叠；折叠时不挂载卡片正文，展开批量卡后显示输入数量与逐条结果。
- Board 任务结果卡将各工作流状态列顶端对齐，并为每列设置 32 rem 最大高度与独立纵向滚动。鼠标位于任意状态列上时仍可横向滚动整个状态区域，同时在视觉上隐藏横向和纵向滚动条。
- 没有可用且已选择的模型时不能发送消息。
...
- 不预先知道或枚举项目 ID 时，也可通过任务关键词直接取得匹配任务 ID、所属项目和状态。
- 置顶上限、归档流程、深链接和活动运行监视器在刷新前后保持一致。
- 包含数百次工具调用的持久化会话可在刷新时回放，而不会挂载全部卡片正文；新流不会使已稳定的历史消息重复渲染。
- 单元素批次与单条写入语义一致；部分成功的批次保留成功结果、标记失败索引，并展示准确汇总。
