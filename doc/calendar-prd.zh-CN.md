...
- 仅在用户明确要求时为当前用户创建 1 至 50 条事件；单条事件使用仅含一个元素的 `items` 数组。
- 开始和结束时间使用配置的系统时区本地时间。
- 与 UI 共享颜色、重复、间隔、提醒和时间校验规则。

### `calendar_update_event`

- 使用必填 `items` 数组更新通过日程查询得到的 1 至 50 条当前用户事件。
- 省略字段保持不变；空描述或地点清空对应字段。
- 修改作用于整个重复系列。
- 不能修改 Board 任务。

创建和更新批次按顺序执行，返回逐条成功或失败，并在单条失败后继续处理。新的执行不接受旧的顶层单事件输入。
...
- Assistant 事件写入支持最多 50 条的单元素和多元素批次，并准确展示部分成功结果。
