...
- 允许创建者把文档的某一版快照发布为不可猜测的链接，任何人无需登录即可阅读。
...
- 跟随文档实时变化、可枚举、可列出，或允许评论和编辑的公开链接。
- 链接访问密码、访问次数统计，或关闭整个实例分享能力的总开关。
...
17. 发布会固定对外公开的 revision。文档继续编辑时链接仍然只提供那一版快照；只有明确的重新发布会推进它，修改链接的过期时间不会。
18. 只有文档创建者可以发布、重新发布、设置过期或撤销链接，与归档和删除一致；项目成员身份不带来任何分享权限。
19. 链接在过期、被撤销，或文档被归档、删除的那一刻立即失效。这几种情况的响应完全相同，链接不会透露究竟发生了哪一种。
20. 已发布的快照只提供它引用到的附件；发布之后上传的附件保持私有。
...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `doc` | relation → docs | 必填、唯一、级联删除 |
| `slug` | text | 唯一的 22 位小写字母数字公开标识 |
| `revision` | number | 已发布的 revision，正文从 `doc_versions` 读取 |
| `expires` | date | 可选；为空表示链接不过期 |
| `created_by` | relation → users | 执行发布的创建者 |
| `created`、`updated` | autodate | 记录时间戳 |

分享独立成表，使发布、过期和撤销都不触碰文档，也不产生版本。Owner 通过 PocketBase list/view 规则读取自己的分享；所有写入都走 Docs API，客户端既不能指定 slug，也不能手工固定 revision。撤销即删除记录，因此再次发布会签发新的 slug，而不是让旧链接复活。
...
只有文档创建者可以移动文档。文档可在 `My documents`、创建者自己的一级个人文件夹，以及创建者拥有 Owner、Admin 或 Member 权限的项目之间移动。移动只改变位置和访问范围，不改变正文 revision，也不创建版本。文档离开项目时，同一事务会解除它与该项目所有任务的关联，避免保留跨项目文档关系。
...
- 创建者从文档工具栏分享：对话框负责创建链接、复制链接、在新标签页打开链接、对比已发布 revision 与当前 revision、按需发布最新版本、设置或清除过期时间，以及撤销。
- 分享的文档在认证外壳之外的 `/s/{slug}` 打开，文档周围不加任何页面外壳：正文自己的标题就是页面标题，文档标题只用于浏览器标签页。链接不可用时只说明不可用。
- 分享的 Markdown 提供两项阅读辅助：顶部的滚动进度条，以及宽屏下正文旁的大纲。大纲取自文档自己的标题，高亮当前章节并支持点击跳转。分享的 HTML 使用与编辑器预览相同的不透明来源沙箱，不提供这两项，因为它的内容对页面不透明。
...
`/api/public/docs` 下的两个路由是匿名访问者唯一能到达的接口。它们把 slug 解析为一份已发布快照，返回标题、类型、正文、revision 和发布时间；Owner、项目、文件夹和编辑者身份都不出服务器。之所以是自定义接口，是因为内建规则无法完成"用 slug 换取另一个集合的记录、校验过期、裁剪私有字段"这一串动作。附件按 slug 代理，是因为受保护文件需要匿名访问者拿不到的 file token。其余所有接口都要求 `users` 认证。对于无权访问的个人/项目文档，适用接口使用未找到语义，避免泄露记录是否存在。

个人文件夹通过 PocketBase `/api/collections/doc_folders/records` CRUD。文档位置变更统一使用 Docs 移动接口，使个人与项目目标共用同一套权限和任务关联清理规则。
...
- `docs_move`：仅在用户明确要求整理或移动时，通过必填 `items` 数组将 1 至 50 篇由当前用户创建的文档在 `My documents`、已有个人文件夹和可编辑项目之间移动；单条移动使用一个元素，并拒绝旧的顶层单文档输入。离开项目会自动解除任务关联，各项按顺序执行并独立返回结果。
...
`board_tasks.documents` 最多关联 20 篇文档。服务端只接受 `project` 与任务项目一致的文档。Board 选择器列出活动项目文档，任务活动以文档标题记录关联变化；删除文档或将文档移出项目时自动移除任务关系，但不删除任务。
...
- 文档创建者可在个人位置与可编辑项目之间移动文档而不改变 revision；离开项目时以原子方式解除原项目任务关联。
