...
- 使用 MDXEditor 提供接近普通文档的所见即所得编辑体验，不要求用户直接编辑 Markdown 源码。
...
首版使用 `@mdxeditor/editor`。

MDXEditor 负责将 Markdown 呈现为所见即所得的文档编辑界面；应用只接收其输出的 Markdown 字符串，不保存 Lexical 内部状态。
...
- Source 模式使用 MDXEditor 的 source view，不额外引入独立 Markdown 编辑器。
- 从 Source 切回 Rich Text 时，如果 Markdown 无法被 MDXEditor 安全解析，保留 Source 内容并显示错误，不丢弃或自动修正用户输入。
...
- 正文无卡片边框，居中限制合理阅读宽度，通过左右内边距、行高及标题/段落/列表间距保持清爽。
...
3. 用户可以在 MDXEditor 中以所见即所得方式编辑标题、段落、列表、引用、链接、表格和代码块。
